...
elif [[ -e /etc/centos-release || -e /etc/redhat-release || -e /etc/oracle-release || -e /etc/almalinux-release  || -e /etc/rocky-release ]]; then
    setenforce 0
...
if [ "$OS" == "debian" ] || [ "$OS" == "ubuntu" ]; then
    apt-get install -y zip wget apt-transport-https bzip2 curl figlet gpg sudo
elif [ "$OS" == "centos" ] || [ "$OS" == "oracle" ] || [ "$OS" == "almalinux" ] || [ "$OS" == "rocky" ] || [ "$OS" == "ol" ]; then
    if [ "$codename" -ge 8 ]; then
        dnf install https://dl.fedoraproject.org/pub/epel/epel-release-latest-$codename.noarch.rpm
        dnf install -y zip wget apt-transport-https bzip2 curl figlet sudo
    elif [ "$codename" -eq 7 ]; then
        sudo rpm -ivh https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
        subscription-manager repos --enable "rhel-*-optional-rpms" --enable "rhel-*-extras-rpms"
        yum update
        yum install -y zip wget apt-transport-https bzip2 curl figlet sudo
    fi
fi
